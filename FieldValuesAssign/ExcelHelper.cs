...
namespace FieldValuesAssign
{
    ...
    public class ExcelHelper
    {
        ...
        public ExcelHelper()
        {
            ...
            this.directory = Path.GetDirectoryName(dllPath);
            this.xlsxPath = Path.Combine(this.directory, "template.xlsx");
            this.loadConfig();
            this.loadData();
        }
        ...
        public void open()
        {
            Process.Start(this.xlsxPath);
        }

        // Token: 0x06000037 RID: 55 RVA: 0x00003CB8 File Offset: 0x00001EB8
        public void loadConfig()
        {
            ExcelQueryFactory f = new ExcelQueryFactory(this.xlsxPath);
        ...
        {
            ExcelQueryFactory f = new ExcelQueryFactory(this.xlsxPath);
            string sheetName = this.config["表名"];
        ...
        public Dictionary<int, string> getFieldNames()
        {
            ...
            int temp;
            var r = from row in this.config
                    where row.Key.StartsWith(searchKey) && int.TryParse(row.Key.Replace(searchKey, ""), out temp)
                    select new
                    {
                        Value = row.Value,
        ...
        public string getFieldsType()
        {
            ...
            IEnumerable<string> r = (from row in this.config
